...
export class AlumnosService {
  ...
  public actualizarAlumno(data: any): Observable<any> {
    ...
    // Si recibimos un id en el objeto, usar la ruta RESTful /alumnos/{id}/
    if (data && (data.id || data.user?.id)) {
      const id = data.id || data.user.id;
      return this.http.put<any>(`${environment.url_api}/alumnos/${id}/`, data, { headers });
    }
    // Fallback al endpoint sin id (acepta id en body o query)
